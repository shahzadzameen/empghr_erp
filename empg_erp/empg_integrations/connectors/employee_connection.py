...
class EmployeeConnection(BaseConnection):
	...
	def get(self, remote_objectname, fields=None, filters=None, start=0, page_length=10):
		...
		query = ("""
			SELECT 
				...
				IF(!es.isactive OR es.emp_status_name='Left', 'Left', 'Active') as `status`,
				CASE
					WHEN (!es.isactive OR es.emp_status_name='Left') AND es.date_of_leaving is not null THEN es.date_of_leaving
					WHEN (!es.isactive OR es.emp_status_name='Left') AND (es.date_of_leaving is null OR es.date_of_leaving = '' OR es.date_of_leaving = '000-00-00') THEN '2022-01-01'
					WHEN (es.isactive OR es.emp_status_name!='Left') THEN NULL
		...
		for data in cursor:
			...
			if frappe.db.exists('User', data['email']) == None:
				del data['email']
