...
class EmployeePermAddressConnection(BaseConnection):
	...
	def get(self, remote_objectname, fields=None, filters=None, start=0, page_length=10):
		...
		query = ("""
			SELECT 
                ...
                ecd.perm_streetaddress as street_address,
                c.country_name as country,
                s.state_name as state,
                city.city_name as city,
                ecd.perm_pincode as postal_code,
                'Permanent' as `type`
            FROM
                ...
                INNER JOIN
                tbl_countries c ON c.id = ecd.perm_country
                INNER JOIN
                tbl_cities city ON city.id = ecd.perm_city
                INNER JOIN
                tbl_states s ON s.id = ecd.perm_state
            WHERE
                ecd.isactive = 1;""")
